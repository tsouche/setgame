...
class Step:
    ...
    def serialize(self):
        """
        This method return a JSON object representing the Step. It will enable 
        ...
        backend and various fronts (apps, web...) over the network.
        """
        stepJSON = {}
        # store generic values
        stepJSON["__class__"] = "SetStep"
        stepJSON["turnCounter"] = str(self.turnCounter)
        stepJSON["playerID"] = str(self.playerID)
        stepJSON["playerName"] = self.playerName
        # store the 'Pick' list
        stepJSON["pick"] = []
        i = 0
        while i < len(self.pick):
            c = self.pick[i]
            stepJSON["pick"].append(str(i).zfill(2) + "-" + str(c).zfill(2))
            i += 1
        ...
        while i < len(self.table):
            c = self.table[i]
            stepJSON["table"].append(str(i).zfill(2) + "-" + str(c).zfill(2))
            i += 1
        # store the 'Used' list
        stepJSON["used"] = []
        i = 0
        while i < len(self.used):
            c = self.used[i]
            stepJSON["used"].append(str(i).zfill(2) + "-" + str(c).zfill(2))
            i += 1
        # store the 'Set' list
        stepJSON["set"] = []
        i = 0
        while i < len(self.set):
            pos = self.set[i]
            c = self.table[pos]
            stepJSON["set"].append(str(pos).zfill(2))
            i += 1
        return stepJSON
    ...
    def deserialize(self, objJSON):
        ...
        def sortJSONList(listJSON):
            """
            This method read a JSON iterable as generated by the 'serialize' 
            function above. Each element is a string with a for "xx-yy" where 
            'xx' is the index and 'yy' the value to be stored. We first sort 
            according to "xx" value, and then we extract "yy" to put it in the 
